...
class RobertaEnt(Base_Connector):
    def __init__(self, args):
        ...
        config = RobertaConfig.from_pretrained(  '/home/yedeming/bert_models/roberta-base' )


        # config.bert_model_name = 'roberta-base'
        # config.entity_emb_size = 768
        # config.is_decoder = False
        # self.model  = RobertaEntForMaskedLM.from_pretrained((config) 
        # self.model.load_state_dict(torch.load(os.path.join(args.luke_model_dir, 'pytorch_unk4.pt')), strict=False)
        ...
        # self.embeddings = self.model.roberta.embeddings.word_embeddings.weight.data
        self.dim = dim = 768
        embed_path = '/home/yedeming/PELT/wikiembed/'
        self.name2pageid =  json.load(open(embed_path+'name2id.json'))
        self.pageid2id = pickle.load(open(embed_path+'pageid2id.pkl', 'rb'))
        # self.pageid2freq = pickle.load(open(embed_path+'pageid2freq.pkl', 'rb'))
        self.tot_entity_embed = np.memmap(embed_path+'tot_embed_mask_varynorm.memmap', mode='r',  dtype=np.float16)
    ...
    def get_batch_generation(self, sentences_list, logger=None, try_cuda=True, sub_labels=None, sub_ids=None):
        ...
        for masked_inputs_list, sub_label, sub_id in zip(sentences_list, sub_labels, sub_ids):
            ...
            for idx, masked_input in enumerate(masked_inputs_list):
                ...
                tokens = self.tokenizer.tokenize(masked_input, add_prefix_space=self.add_prefix_space)   # 是否加pre-space
                ...
                attention_mask = torch.zeros((self.max_seq_length+entity_K, self.max_seq_length+entity_K), dtype=torch.int64)
                attention_mask[:L, :L] = 1
                if self.kind!=4: 
                    for i, (page_id, sub_s, sub_e) in enumerate(mentions):
                        ...
                        if page_id in self.pageid2id:
                            ...
                            entity_position_id = list(range(sub_s+self.position_plus, sub_e+self.position_plus))
                            entity_position_id = entity_position_id[:self.max_mention_length]
                            entity_position_id += [-1]*(self.max_mention_length-len(entity_position_id))

                            entity_embeddings.append(entity_embedding)
                            entity_position_ids.append(entity_position_id)

                            attention_mask[self.max_seq_length+i, self.max_seq_length+i] = 1
                            if self.kind==1:
                                attention_mask[:L, self.max_seq_length+i] = 1
                            elif self.kind==2:
                                attention_mask[sub_s:sub_e, self.max_seq_length+i] = 1
                            elif self.kind==3:
                                attention_mask[self.max_seq_length+i, sub_s:sub_e] = 1
                                attention_mask[sub_s:sub_e, self.max_seq_length+i] = 1
                ...
                if self.kind==0: 
                    attention_mask[:L, self.max_seq_length:self.max_seq_length+len(mentions)] = 1
                    attention_mask[self.max_seq_length:self.max_seq_length+len(mentions), :L] = 1
                    attention_mask[self.max_seq_length:self.max_seq_length+len(mentions), self.max_seq_length:self.max_seq_length+len(mentions)] = 1
                ...
                while len(entity_embeddings) < entity_K:
                    ...
                    entity_position_ids.append([-1] * self.max_mention_length)
        ...
        attention_mask_list = torch.stack(attention_mask_list, dim=0)#torch.tensor(attention_mask_list, dtype=torch.int64)
